...
description: Andy の作成手順「agency-operations」（self-evolving-agent から自動同期）: 並列 agent を agency で安全に運用する手順(agency 運用知見)
---
...
oven-android では `gh pr edit`(assignee / reviewer / body の編集)が **Projects classic 廃止由来の `projectCards` GraphQL エラー**で失敗する。
...
- アプリモジュールが複数ある場合(例 `:app` と `:app-TimeTree`)、片方だけ見て他方のテストを見落とさない。壊れた参照は別モジュールの test 配下にいることがある。
...
- ローカルで再現できない領域(secrets 必須のアプリモジュール等。例 `:app-TimeTree` は `secrets.properties` 必須でローカル config 不可)は**最初から「CI に委ねる」と宣言**し、緑を断定しない。
