...
1. **引数**: `$ARGUMENTS` からPR番号を読み取る（`#42` または `42`）
2. **会話コンテキスト**: 引数がなければ、このセッションで直前に扱っていたPRの番号を対象にする（直前に確認・対応していたPR、ユーザーが会話中に挙げた番号など）。レビューを始める前に「コンテキストから #42 を対象にした」と一行で明示し、取り違えていればユーザーがすぐ指摘できるようにする
3. **現在のブランチ**: 引数もコンテキストもなければ現在のブランチのPRを探す

```bash
gh pr view --json number,title,url 2>/dev/null
```

それでも特定できなければ、自分のopen PRを一覧してユーザーに確認する（stack PRのようにopen PRが多いリポジトリでは `--limit 10` だと漏れるため `--limit 30`）:
