...
- 過去に同じ議論が出た／今後再燃しそう
- 外部ステークホルダー（PM・パートナー等）の判断に影響する技術選択
...
| 論点 | [「〇〇 vs 〇〇」で 1 行] |
| 作成日 | [今日の日付] |
| 委任度 | ユーザー決定 / チーム提案・ユーザー承認 / チーム決定（後報）|
| ステータス | 草案 / レビュー中 / 決定済み / 廃止 |
| 決定期限 | [YYYY-MM-DD — なぜその日までか 1 行]（Lesson C：平日確認）|

**委任度の 3 分類（EM として使い分ける）:** ユーザー決定＝アーキ全体・チームパターン変更・外部影響あり／チーム提案・ユーザー承認＝実装詳細・モジュール内設計で ユーザーはゲート／チーム決定（後報）＝影響限定でチーム自律、ADR は事後報告。
