...
- `node agency.ts run <id> <repo> <prompt...>` — 新規 agent tab を inbox 経由で立てる(**要 git repo**。base は `detectBase` が repo ごとに自動判定。oven-android は最新 milestone。worktree 隔離あり)。
...
- `/agency new <repo> <指示...>` — repo は basename(`oven-android` 等、`~/Documents/<name>` に解決)か絶対パス。id は repo basename を既定にし、衝突する場合だけ連番。**worktree 隔離ありの実作業向け**。
...
- agent tab の base ブランチ規約は `repo-base.json`(例 `oven-android` = 最新 milestone)で管理。新 repo で base を変えたいときはそこに追記。
